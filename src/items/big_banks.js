...
const VERBAL_ANALOGIES = [
  ...
  { a1: "Thermometer", b1: "Temperature", a2: "Scale", correct: "Weight", distractors: ["Balance", "Metal", "Length"] },
...
function renderGlyph(cell){
  ...
  for (let i = 0; i < cell.count; i++){
    ...
    glyphs.push(shapeSvg(cell, cx, cy, 16));
  }
  return `<svg viewBox="0 0 ${size} ${size}" width="${size}" height="${size}" class="glyphSvg">
    <defs>
      <linearGradient id="glyph-bg-${fluidVisualKey(cell)}" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="${cell.accent || "rgba(255,255,255,0.16)"}"/>
        <stop offset="100%" stop-color="rgba(255,255,255,0.04)"/>
      </linearGradient>
    </defs>
    <rect x="5" y="5" width="${size - 10}" height="${size - 10}" rx="20" fill="url(#glyph-bg-${fluidVisualKey(cell)})" stroke="${cell.tint || "rgba(232,238,252,0.14)"}"/>
    ${glyphs.join("")}
  </svg>`;
}

function shapeSvg(cell, cx, cy, r){
  const uid = `${cell.shape}-${cell.fill}-${cell.rot}-${Math.round(cx * 7 + cy * 11 + r * 13)}`;
...
function makeVerbalBank(rng, n){
  ...
  for (let i = 0; i < n; i++){
    ...
    if (t === "analogy"){
      ...
      const prompt = `${tpl.a1} is to ${tpl.b1} as ${tpl.a2} is to:`;
    ...
    if (t === "classification"){
      ...
      items.push({ id: `V${i + 1}`, type: "mcq", domain: "verbal", title: "Classification", prompt: "Which word does not belong with the others?", choices, answer: tpl.correct, blueprint: "classification", irt: { a, b, c, k }, meta: { kind: "classification" } });
      continue;
    }
    ...
    const choices = shuffle(rng, [tpl.correct, ...tpl.distractors]);
    items.push({ id: `V${i + 1}`, type: "mcq", domain: "verbal", title: "Logical inference", prompt: `${tpl.premises.join(" ")} Which conclusion follows?`, choices, answer: tpl.correct, blueprint: "logic", irt: { a, b, c, k }, meta: { kind: "logic" } });
  }
  return items;
}
...
function makeQuantBank(rng, n){
  const items = [];
  const kinds = ["series", "algebra", "ratio", "rate", "comparison"];
  ...
  for (let i = 0; i < n; i++){
    ...
    if (kind === "series"){
      ...
      const stepA = randInt(rng, 2, 6);
      const stepB = stepA + randInt(rng, 1, 4);
      const seq = [start, start + stepA, start + stepA + stepB, start + stepA + stepB + stepA, start + stepA + stepB + stepA + stepB];
      const answer = String(seq[seq.length - 1] + stepA);
      const choices = shuffle(rng, [answer, String(Number(answer) + stepB - stepA), String(Number(answer) - stepA), String(Number(answer) + stepA + stepB)]);
      items.push({ id: `Q${i + 1}`, type: "mcq", domain: "quant", title: "Number series", prompt: `What comes next? ${seq.join(", ")}, ?`, choices, answer, blueprint: "series", irt: { a, b, c, k }, meta: { kind: "series" } });
      continue;
    }
    ...
    if (kind === "algebra"){
      ...
      const offset = randInt(rng, 1, 12);
      const total = coef * x + offset;
      const answer = String(x);
      const choices = shuffle(rng, [answer, String(x + 1), String(Math.max(1, x - 1)), String(x + 2)]);
      items.push({ id: `Q${i + 1}`, type: "mcq", domain: "quant", title: "Linear equation", prompt: `Solve for x: ${coef}x + ${offset} = ${total}`, choices, answer, blueprint: "algebra", irt: { a, b, c, k }, meta: { kind: "algebra" } });
      continue;
    }

    if (kind === "ratio"){
      const flour = randInt(rng, 2, 6);
      const sugar = flour + randInt(rng, 1, 5);
      const newFlour = flour * randInt(rng, 2, 3);
      const answerVal = (newFlour * sugar) / flour;
      const answer = toNumberChoice(answerVal);
      const choices = shuffle(rng, [answer, toNumberChoice(answerVal + flour), toNumberChoice(answerVal - sugar / flour), toNumberChoice(answerVal + sugar / flour)]);
      items.push({ id: `Q${i + 1}`, type: "mcq", domain: "quant", title: "Proportional reasoning", prompt: `A recipe uses ${flour} cups of flour for ${sugar} cups of sugar. If you use ${newFlour} cups of flour, how many cups of sugar keep the same ratio?`, choices, answer, blueprint: "ratio", irt: { a, b, c, k }, meta: { kind: "ratio" } });
      continue;
    }

    if (kind === "rate"){
      const pages = randInt(rng, 12, 24);
      const minutes = randInt(rng, 3, 8);
      const multiplier = randInt(rng, 2, 4);
      const targetPages = pages * multiplier;
      const answerVal = minutes * multiplier;
      const answer = String(answerVal);
      const choices = shuffle(rng, [answer, String(answerVal + minutes), String(Math.max(1, answerVal - minutes)), String(multiplier)]);
      items.push({ id: `Q${i + 1}`, type: "mcq", domain: "quant", title: "Rate reasoning", prompt: `A printer produces ${pages} pages in ${minutes} minutes at a constant rate. How many minutes does it take to produce ${targetPages} pages?`, choices, answer, blueprint: "rate", irt: { a, b, c, k }, meta: { kind: "rate" } });
      continue;
    }

    const base = randInt(rng, 20, 42);
    const delta = randInt(rng, 4, 10);
    const quantityA = base + delta;
    const quantityB = (base * 2) - delta;
    const answer = quantityA === quantityB ? "The two quantities are equal" : (quantityA > quantityB ? "Quantity A" : "Quantity B");
    const choices = ["Quantity A", "Quantity B", "The two quantities are equal", "Cannot be determined"];
    items.push({ id: `Q${i + 1}`, type: "mcq", domain: "quant", title: "Quantitative comparison", prompt: `Compare the two quantities. Quantity A: ${base} + ${delta}. Quantity B: (${base} x 2) - ${delta}.`, choices, answer, blueprint: "comparison", irt: { a, b, c, k }, meta: { kind: "comparison" } });
  }
...
function makeSpatialBank(rng, n){
  ...
  for (let i = 0; i < n; i++){
    ...
    items.push({
      ...
      prompt: "Which option shows the same shape rotated in space, not mirrored?",
...
function makeSpatialFigure(rng){
  const points = [[18, 18], [52, 18], [52, 30], [70, 30], [70, 58], [46, 58], [46, 74], [24, 74], [24, 46], [18, 46]];
  const marker = choice(rng, [{ x: 30, y: 26 }, { x: 58, y: 36 }, { x: 36, y: 64 }]);
  return { points, marker, tint: choice(rng, ["rgba(125,211,252,0.95)", "rgba(249,168,212,0.95)", "rgba(253,186,116,0.95)"]) };
}
...
function figureSvg(shape, rotation, mirrored, framed = false){
  ...
  const box = framed ? `<rect x="6" y="6" width="84" height="84" rx="20" fill="rgba(255,255,255,0.04)" stroke="rgba(232,238,252,0.16)"/>` : "";
  return `<svg viewBox="0 0 96 96" width="96" height="96">${box}<g transform="rotate(${rotation} 48 48)"><polygon points="${polygon}" fill="rgba(232,238,252,0.14)" stroke="${shape.tint}" stroke-width="2.6"/><circle cx="${markerX}" cy="${shape.marker.y}" r="4.5" fill="${shape.tint}"/></g></svg>`;
}
...
function makeSymbolSearchBank(rng, n){
  ...
  for (let i = 0; i < n; i++){
    ...
    items.push({ id: `SS${i + 1}`, type: "speed_symbol", domain: "speed_symbol", title: "Symbol search", prompt: "Does the target pair appear in the row?", target, row, answer: include ? "YES" : "NO", irt });
  }
  return items;
}
...
function makeCodingBank(rng, n){
  ...
  for (let i = 0; i < n; i++){
    ...
    items.push({ id: `CD${i + 1}`, type: "speed_coding", domain: "speed_coding", title: "Coding", prompt: "Enter the digits that correspond to the symbols (use the key).", keyPairs, seq, answer: seq.map((s) => keyPairs.find((p) => p.sym === s).dig).join(""), irt });
  }
  return items;
}
...
function makeDigitSpanBank(rng, n){
  ...
  for (let i = 0; i < n; i++){
    ...
    items.push({ id: `WM${i + 1}`, type: "digitspan", domain: "wm", title: direction === "backward" ? "Digit span (backward)" : "Digit span", prompt: direction === "backward" ? "Remember the digits and type them backward." : "Remember the digits and type them in order.", digits, direction, blueprint: direction, showMs: 900 + Math.max(0, 1200 - len * 70), irt: { a: Math.min(2.0, a + 0.4), b, c: 0, k: 10 }, meta: { len, direction } });
  }
  return items;
}
